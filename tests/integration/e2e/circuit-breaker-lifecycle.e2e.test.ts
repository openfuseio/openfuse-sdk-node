...
function getTestMode(metricType: TMetricType): TTestMode {
  const hasFixtures = metricType === 'failure-rate' ? hasFailureRateFixtures() : hasLatencyFixtures()
...
async function setupTest(
  ...
  if (mode === 'fixtures') {
    ...
    if (breaker.state !== 'closed') {
      ...
      breaker = await apiClient.updateBreakerState(system.id, breaker.id, 'closed', 'E2E test reset')
    }
  ...
  const policyConfig = metricType === 'failure-rate'
...
describe.skipIf(!canRunTests('failure-rate'))('E2E: Circuit Breaker Lifecycle', () => {
  ...
  it('should trip breaker when failure rate exceeds threshold', async () => {
  ...
  it('should transition to half-open after probe interval', async () => {
    const client = createSDKClient(sdkTokenProvider, system.slug)
    await client.bootstrap()
    ...
    try {
      ...
      console.log('Breaker state:', { state: breakerState.state, retryAfter: breakerState.retryAfter })
      ...
      while (Date.now() - startTime < TIMING.HALF_OPEN_WAIT_MS) {
        ...
        if (current.state === 'half-open') {
          transitioned = true
          console.log(`Breaker transitioned to half-open after ${(Date.now() - startTime) / 1000}s`)
          break
        }
        await sleep(TIMING.POLL_INTERVAL_MS)
      }
  ...
  it('should close breaker after successful probe in half-open state', async () => {
    const client = createSDKClient(sdkTokenProvider, system.slug)
    await client.bootstrap()
...
describe.skipIf(!canRunTests('failure-rate'))('E2E: Circuit Breaker - onOpen fallback', () => {
  ...
  it('should call onOpen fallback when breaker trips', async () => {
    const client = createSDKClient(sdkTokenProvider, system.slug)
    await client.bootstrap()
...
describe.skipIf(!canRunTests('latency-p95'))('E2E: Circuit Breaker - Latency p95 trip', () => {
  ...
  it('should trip breaker when latency p95 exceeds threshold', async () => {
    const client = createSDKClient(sdkTokenProvider, system.slug)
    await client.bootstrap()
...
describe.skipIf(!canRunTests('failure-rate'))('E2E: Circuit Breaker - Pure withBreaker() flow', () => {
  let sdkTokenProvider: TTokenProvider
  let system: TTestSystem
  let breaker: TTestBreaker
  ...
  it('should block calls after failures trip the breaker', async () => {
    const client = createSDKClient(sdkTokenProvider, system.slug)
    await client.bootstrap()

    try {
      expect(await client.withBreaker(breaker.slug, async () => 'initial-success')).toBe('initial-success')
